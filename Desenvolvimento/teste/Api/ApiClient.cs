...
namespace  TesteWebApi.Api
{
    public partial class ApiClient
    {
        ...
        private void Signin(Uri requestUrl, string username, string password)
        {
            ...
            var response = Task.Run(() => { return  _httpClient.PostAsync(requestUrl.ToString(), null); }).Result;

            response.EnsureSuccessStatusCode();
            var data = response.Content.ReadAsStringAsync().Result;
            LoginToken _return = JsonConvert.DeserializeObject<LoginToken>(data);

            this.token = _return.token;
        }
