...
export class FuseNavigationService {
    ...
    private prepareMenu(menuItem: FuseNavigation[]): FuseNavigation[] {
        const user = this._credentialsService.authenticatedUser;
        if (!user) {
            return [];
        }
        const currentUserRoles = user.roles;
        // TODO: Refatorar
        const hasCurrentUserRoles = role => !!~currentUserRoles.indexOf(role);
        const checkAndFindRoles = x => !x.roles || (x.roles && x.roles.find(hasCurrentUserRoles));
        const checkAndFindSubRoles = m => !m.children || (m.children = m.children.filter(checkAndFindRoles));
        menuItem = menuItem.filter(checkAndFindRoles);
        menuItem = menuItem.filter(checkAndFindSubRoles);
        menuItem = menuItem.filter(m => !m.children || (m.children && m.children.filter(checkAndFindSubRoles)));
        return menuItem;
    }
    ...
    register(key, navigation): void {
        ...
        if (this._registry[key]) {
            console.error(`A navegação com a chave '${key}' já existe. Cancele o registro primeiro ou use uma chave exclusiva.`);

            return;
        }

        navigation = this.prepareMenu(navigation);
